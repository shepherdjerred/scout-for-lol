...
describe.serial("pruneOrphanedPlayers", () => {
  ...
  it("should prune players with no subscriptions and no active competitions", async () => {
    ...
    await prisma.player.create({
      data: {
        ...
        creatorDiscordId: DiscordAccountIdSchema.parse("user1000000000000"),
        createdTime: now,
        updatedTime: now,
      },
    });
  ...
  it("should NOT prune players with subscriptions", async () => {
    ...
    await prisma.player.create({
      data: {
        ...
        subscriptions: {
          create: {
            channelId: DiscordChannelIdSchema.parse("channel1000000000"),
            serverId: DiscordGuildIdSchema.parse("server-1000000001"),
            creatorDiscordId: DiscordAccountIdSchema.parse("user1000000000000"),
            createdTime: now,
            updatedTime: now,
  ...
  it("should NOT prune players with active competition participations", async () => {
    ...
    const competition = await prisma.competition.create({
      data: {
        serverId: DiscordGuildIdSchema.parse("server-1000000001"),
        ownerId: DiscordAccountIdSchema.parse("user1000000000000"),
      ...
      data: {
        ...
        competitionParticipants: {
          create: {
            competitionId: competition.id,
            status: "INVITED",
            invitedBy: DiscordAccountIdSchema.parse("user2000000000000"),
            invitedAt: now,
          },
        },
      },
